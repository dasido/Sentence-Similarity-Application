# -*- coding: utf-8 -*-
'''This is the file used by the CNN_SV model to read the data set and preprocess the data set'''
import numpy as np
import pickle
...
class ReadDataset:
    ...
    def next_batch(self, batch):
        ...
        for i in range(batch):
            ...
            print("datdatdatdatadatdata")
            print(data)
            s1 = self.sentence_to_index(data[1]) #return a list of index of rhe first sen.
            s2 = self.sentence_to_index(data[2]) #return a list of index of rhe second sen.
            l = float(data[3])
            print("s1s1s1s1s1s1s1s1")#after sentence to index
            print(s1)
            print("s2s2s2s2s2s2s2s2s2")
            print(s2)
            print("similiarity")
            print(l)
            vector_x.append(s1)
        ...
        del(vector_y)
        print("xxxxxxxxxxxxxxxxxx")#after vector to matrix
        print(x)
